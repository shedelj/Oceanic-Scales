...
void control_chem(uint8_t id)
{
   ...
   switch(leds[id].type)
   {
      case TEMPERATURE: strip.setPixelColor(id, brightness, 0, 0);
                        break;
      default: break;
   } 
}
...
{  
   ...
   int distance =  relevant_value - leds[id].y * (255 / WINDOW_Y) ;
   if(distance + 40 < 0)
   {
     return 0;
   }
